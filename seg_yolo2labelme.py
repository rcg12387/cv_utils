...
def seg_yolo2labelme(img_path, yolo_path, labelme_path, cls_file):
        ...
        if osp.exists(label_name):
            with open(label_name) as f:
                ...
                for label in labels:
                    ...
                    # Points
                    points = np.array([float(x) for x in ann[1:]])
                    points[::2] *= iw
                    points[1::2] *= ih
...
if __name__ == "__main__":
    ...
    seg_yolo2labelme(opt.images, opt.yolo, opt.labelme, opt.cls_file)
